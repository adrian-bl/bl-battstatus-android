...
my $wfb  = { size=>72, fsize=>36, fsize_100=>34, font=>FONTPATH, o=>0, invert=>0, name=>'wfb' };
my $bfb  = { size=>72, fsize=>36, fsize_100=>34, font=>FONTPATH, o=>0, invert=>1, name=>'bfb' };
my $wfs  = { size=>72, fsize=>28, fsize_100=>28, font=>FONTPATH, o=>0, invert=>0, name=>'wfs' };
my $bfs  = { size=>72, fsize=>28, fsize_100=>28, font=>FONTPATH, o=>0, invert=>1, name=>'bfs' };
...
sub assemble_font {
	...
	foreach my $x (-2..2) {
		foreach my $y (-2..2) {
			$im->Annotate(text=>"$num", fill=>$tc_out, font=>$config->{font}, pointsize=>$fsize+0, antialias=>'true', gravity=>"Center", x=>$xoff+$x, y=>$y);
		}
	}
	
	$im->Annotate(text=>"$num", fill=>'grey', font=>$config->{font}, pointsize=>$fsize+0, antialias=>'true', gravity=>"Center", x=>$xoff, y=>0); # creates somewhat of a gardient
	$im->Annotate(text=>"$num", fill=>$tc_in, font=>$config->{font}, pointsize=>$fsize+0, antialias=>'true', gravity=>"Center", x=>$xoff, y=>0);
